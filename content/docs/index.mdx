...
<CardGroup>
  <Card title="Spec-first generation" icon="*">
    Quell reads docstrings, Pydantic models, and bug descriptions. It extracts requirements, not implementation.
  </Card>
  ...
  <Card title="Privacy first" icon="@">
    Code is never sent to any server unless you configure an LLM key. LLM is only called for complex unstructured specs — most requirements are handled by the rule engine with no network call.
  </Card>
